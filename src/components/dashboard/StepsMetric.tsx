...
export function StepsMetric({
  ...
  const totalTargetSteps = dailyTarget * dateRangeInfo.daysInPeriod;
  const remainingSteps = totalTargetSteps - totalSteps;
  const averageRemainingSteps = dateRangeInfo.remainingDaysForLogs > 0
    ? Math.round(remainingSteps / dateRangeInfo.remainingDaysForLogs)
    : 0;
